...
export class ShowgradsComponent implements OnInit {
  grads: Grad[];
  rowData: Grad[];
  headElements=["Employee Code","Name","Email","College", "Location ", "Date of Joining", "Batch","DoJ Reconfirmation(Y/N)",
  "Remarks", "Course","Branch","CGPA", "Personal Contact", "Personal Email",
  "DoB", "Native Place", "Current Place", "Parent Contact", "Postal Address", "CV Link",
  "Action"];
  sortable=["Employee Code", "Name", "College", "Location", "Date of Joining", "CGPA", "DoB", "Current Place"]
  ...
  getSortable(head: string){
    console.log(head);
    if (head in this.sortable){
      return head;
    }
    else {
      return null;
